...
// Storage emits flat tool calls; AG-UI's RunAgentInputSchema requires nested.
// Rewrite at the hydration boundary so the next runAgent() validates.
export function normalizeStoredMessage(raw: unknown): unknown {
  if (!raw || typeof raw !== 'object') return raw;
  const msg = raw as Record<string, unknown>;
  if (msg.role !== 'assistant' || !Array.isArray(msg.toolCalls)) return msg;
  return {
    ...msg,
    toolCalls: msg.toolCalls.map(tc => {
      if (!tc || typeof tc !== 'object') return tc;
      const obj = tc as Record<string, unknown>;
      if (obj.function && typeof obj.function === 'object') return obj;
      return {
        id: obj.id,
        type: 'function',
        function: {
...
export function projectTimeline(messages: Message[]): TimelineItem[] {
  ...
  for (const msg of messages) {
    if (msg.role === 'tool') toolMessages.set(msg.toolCallId, msg);
  }
...
// Message.content is `string | Part[]`; we only render text parts.
