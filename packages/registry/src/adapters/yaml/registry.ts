...
export class YamlRegistryRepository implements RegistryRepository {
  ...
  protected readonly source: YamlSource;
  ...
  constructor(opts: YamlRegistryRepositoryOptions) {
    ...
    const getIndex = (): RegistryIndexes => {
      if (!this.indexes) {
        throw new Error('YamlRegistryRepository: refresh() not called');
      }
  ...
  private async doRefresh(opts: { initial: boolean }): Promise<void> {
    try {
      const res = await this.source.read(this.etag ? { ifNoneMatch: this.etag } : undefined);
      ...
      this.lastRefreshedAt = Date.now();
      console.info(
        `[registry/yaml] loaded ${this.source.describe()} etag=${this.etag ?? 'none'}`,
      );
    } catch (err) {
      if (opts.initial) {
        throw err;
      ...
      console.warn(
        `[registry/yaml] refresh failed (staying on cached): ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }

  private maybeRefreshInBackground(): void {
    if (Date.now() - this.lastRefreshedAt < REFRESH_TTL_MS) return;
  ...
  private async readRaw(): Promise<{ text: string; etag: string | undefined }> {
    const res = await this.source.read();
    if (res.kind !== 'ok') {
      throw new Error(`unexpected read result: ${res.kind}`);
    }
  ...
  private async writeRaw(text: string): Promise<{ etag: string | undefined }> {
    ...
    const put = await this.source.write(text);
