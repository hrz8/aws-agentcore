...
export class AgentSessionManager extends SessionManager {
  ...
  initAgent(agent: LocalAgent): void {
    ...
    // AG-UI's clone helper copies only {model, tools}, so plugin-registered
    // hooks on the template never fire on the clone. Re-run initAgent per
    // clone to attach the plugin's BeforeInvocation (XML injection) hook.
    ...
    if (this._beforeInvocation) {
      ...
      // SDK_FIRST so this runs before the plugin's XML injection hook —
      // otherwise a same-turn skills refresh lands too late for the prompt.
